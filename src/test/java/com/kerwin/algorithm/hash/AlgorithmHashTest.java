...
import com.kerwin.algorithm.hash.cache.LRUCacheByLinkedHashMap;
...
public class AlgorithmHashTest {
    ...
    /**
     * 基于JDK LinkedHashMap实现LRU缓存机制
     */
    @Test
    void testLRUCacheByLinkedHashMap(){
        LRUCacheByLinkedHashMap cache = new LRUCacheByLinkedHashMap(2);
        cache.put(1, 1); // 缓存是 {1=1}
        cache.put(2, 2); // 缓存是 {1=1, 2=2}
        log.info("get(1)={}",cache.get(1));     // 返回 1
        cache.put(3, 3); // 该操作会使得关键字 2 作废，缓存是 {1=1, 3=3}
